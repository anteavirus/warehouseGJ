...
public class StorageArea : MonoBehaviour
{
    ...
    private void CreateSingleVisualItem(int positionIndex)
    {
        ...
        visualItem.transform.localScale = UsefulStuffs.Divide(Vector3.one, scaleOffset); // TODO: this doesn't look right. fuck it up
    ...
    private void OnDrawGizmos()
    {
        ...
        if ((itemPositions == null || itemPositions.Length == 0) && !isPlaying)
        {
            ...
            {
                Transform child = transform.GetChild(i);
                if (child != null && (child.name.StartsWith("ItemPosition") || child.CompareTag("ItemPosition")))
                {
                    positions.Add(child);
                }
            }
